...
Usage:
    python tune_sarimax.py --data data/SeoulBikeData.csv
...
def tune_sarimax(
    ...
    max_folds : int
        Number of CV folds for validation (keep small)
    max_train_size : int
        Maximum training observations
    seasonal : bool
    ...
    # Keep only covariates that exist in the dataframe
    ...
    for fold_idx in range(min(max_folds, len(splits))):
        ...
        # Limit training size
        if len(y_train_fold) > max_train_size:
            y_train_fold = y_train_fold[-max_train_size:]
            X_train_fold = X_train_fold[-max_train_size:]
...
def main():
    ...
    parser.add_argument(
        ...
        type=int,
        default=1000,
        help='Maximum training observations'
    )
    parser.add_argument(
        '--seasonal-period',
        type=int,
